...
Faye.extend(Faye, {
        ...
        each: function (object, callback, scope) {
                if (object instanceof Array) {
                        for (var i = 0, n = object.length; i < n; i++) {
                                if (object[i] !== undefined) callback.call(scope || null, object[i], i);
                        }
                } else {
                        for (var key in object) {
                                if (object.hasOwnProperty(key)) callback.call(scope || null, key, object[key]);
                        }
                }
        },
        ...
        filter: function (array, callback, scope) {
                ...
                this.each(array, function () {
                        if (callback.apply(scope, arguments)) result.push(arguments[0]);
                });
                return result;
        },
        ...
        toJSON: function (object) {
                if (this.stringify) return this.stringify(object, function (key, value) {
...
Faye.Deferrable = {
        callback: function (callback, scope) {
                ...
                if (this._deferredStatus === 'succeeded') return callback.apply(scope, this._deferredArgs);
        ...
        errback: function (callback, scope) {
                ...
                if (this._deferredStatus === 'failed') return callback.apply(scope, this._deferredArgs);
...
Faye.extend(Faye.Channel, {
        ...
        isValid: function (name) {
                return Faye.Grammar.CHANNEL_NAME.test(name) || Faye.Grammar.CHANNEL_PATTERN.test(name);
        },
        ...
        Set: Faye.Class({
                ...
                getKeys: function () {
                        var keys = [];
                        Faye.each(this._channels, function (k, v) {
                                keys.push(k)
                        });
                        return keys;
                },
...
Faye.Client = Faye.Class({
        ...
        initialize: function (endpoint, options) {
                ...
                if (Faye.Event) Faye.Event.on(Faye.ENV, 'beforeunload', this.disconnect, this);
        },
        ...
        subscribe: function (channels, callback, scope) {
                if (channels instanceof Array) return Faye.each(channels, function (channel) {
                ...
                this.connect(function () {
                        ...
                        this._send({
                                ...
                                if (!response.successful) return subscription.setDeferredStatus('failed', Faye.Error.parse(response.error));
        ...
        publish: function (channel, data) {
                if (!Faye.Grammar.CHANNEL_NAME.test(channel)) throw new Error("Cannot publish: '" + channel + "' is not a valid channel name");
...
Faye.Transport = Faye.extend(Faye.Class({
        ...
        send: function (message, timeout) {
                this.debug('Client ? sending message to ?: ?', this._client._clientId, this._endpoint, message);
                ...
                if (message.channel === Faye.Channel.HANDSHAKE) return this.flush();
        ...
        receive: function (responses) {
                this.debug('Client ? received from ?: ?', this._client._clientId, this._endpoint, responses);
        ...
        supportedConnectionTypes: function () {
                return Faye.map(this._transports, function (pair) {
                        return pair[0]
                });
        }
});
...
Faye.Event = {
        ...
        detach: function (element, eventName, callback, scope) {
                ...
                while (i--) {
                        ...
                        if ((element && element !== register._element) || (eventName && eventName !== register._type) || (callback && callback !== register._callback) || (scope && scope !== register._scope)) continue;
...
Faye.URI = Faye.extend(Faye.Class({
        ...
        toURL: function () {
                ...
                return this.protocol + this.hostname + ':' + this.port + this.pathname + (query ? '?' + query : '');
        }
}), {
...
Faye.Transport.WebSocket = Faye.extend(Faye.Class(Faye.Transport, {
        ...
        resend: function () {
                var messages = Faye.map(this._messages, function (id, msg) {
                        return msg
                });
...
Faye.Transport.XHR = Faye.extend(Faye.Class(Faye.Transport, {
        request: function (message, timeout) {
                ...
                xhr.onreadystatechange = function () {
                        ...
                        try {
                                if ((status >= 200 && status < 300) || status === 304 || status === 1223) self.receive(JSON.parse(xhr.responseText));
                                else retry();
                        } catch (e) {
                                retry();
                        } finally {
...
Faye.Transport.CORS = Faye.extend(Faye.Class(Faye.Transport, {
        ...
        isUsable: function (endpoint, callback, scope) {
                ...
                if (Faye.ENV.XDomainRequest) return callback.call(scope, true);
